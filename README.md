...
## БД развернута на бесплатном хостинге Aiven (из-за того, что хостинг бесплатный, возможны перебои в подключениях к бд. Поэтому рекомендуется развернуть бд локально и уже потом тестировать проект для надежности). Данные для доступа к БД можно посмотреть в .env файле проекта.
